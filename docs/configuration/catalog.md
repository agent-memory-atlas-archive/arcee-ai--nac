...
NAC ships an embedded model catalog generated from [models.dev](https://models.dev): per-model context windows, maximum output tokens, pricing per 1M tokens, and supported reasoning effort levels. OpenAI's one upstream source is projected to both `openai-responses` and `openai-chat-completions`; the shared metadata stays synchronized while each backend retains its own wire protocol and compatibility profile. Model-only collisions deliberately resolve to Responses. Catalog membership is metadata enrichment, never admission control: an explicit backend plus an arbitrary model id resolves from conservative provider defaults even when models.dev and live `/models` discovery know nothing about it.
...
```json
{
  "overrides": [
    ...
    },
    {
      ...
      "set": { "context_window": 262144, "max_tokens": 32768, "max_completion_tokens": 4096 }
    }
  ]
}
```
...
- `set` accepts `display_name`, `context_window`, `max_tokens`, `cost` (`input`/`output`/`cache_read`/`cache_write` per 1M tokens, plus an optional `tiers` list), `cache_write_1h`, `reasoning`, `image_input`, and `thinking_level_map`. For the `openai-chat-completions` backend it also accepts mutually exclusive `max_completion_tokens` (modern request field) and `legacy_max_tokens` (legacy-compatible request field); neither is inferred from the metadata `max_tokens`. Other wire protocol and adapter quirks are not patchable. Omitted `cost` buckets keep their resolved rates.
